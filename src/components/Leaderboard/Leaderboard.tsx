...
function loadData(store: any) {
  // TODO: Вот в этом месте мы:
  //  а) Выполняем запрос апи напрямую, без диспатча в редьюсере
  //  б) Если данные берутся из бд, мы просто передаем сюда данные юзера из req.userData
  //  Запускать тут диспатчи вообще нет смысла, мы просто можем возвращать данные, а потом их писать в стор
